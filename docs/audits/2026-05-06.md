...
> |---|---|---|---|
> | F1 — artifacts in `src/` (untracked) | BLOCKER | ✅ RESOLVED | files cleaned; `git ls-files` confirms not tracked |
> | F2 — broken regex in `audit-self.yml` rule-to-probe | BLOCKER | ✅ RESOLVED | `bf8b4f5` (regex now `^## R[0-9]+[a-z]?` and `skip_unless ${r}[a-z]*\b`) |
> | F3 — R2 wording drift across 3 sources | MAJOR | ✅ RESOLVED in this PR | Batches A + B (umbrella `feat/audit-fixes-2026-05`) |
> | F4 — Phase 6 of audit prompt self-invalidated | MAJOR | OPEN | tracked separately; not in this umbrella |
> | F5 — `ARCHITECTURE.template.md` naming | MINOR | ✅ RESOLVED in this PR | Batch C — renamed to `ARCHITECTURE.ts-server.md` |
> | F6 — SKILL.md description verb-first | MINOR | ❌ INVALID | rechecked: SKILL.md frontmatter starts with "Use when…" already; only `extension.json` skill description (different field) starts with "Convert" — kept as-is |
>
> This document is a **snapshot in time**. The findings above reflect 2026-05-06 state; do not edit the body. For current state of any specific concern, read the linked commits or open an issue.
