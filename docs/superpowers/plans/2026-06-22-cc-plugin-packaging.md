...
- [ ] **Step 1: Triage table (T-PLUG-A + T-PLUG-B).** For each of the 15 `.claude/hooks/*.sh`, classify: **SHIP** (consumer-relevant) vs **INTERNAL** (do not ship); and for each SHIP hook, classify every path it references as **plugin-data** (`${CLAUDE_PLUGIN_ROOT}`) or **project-data** (`$CLAUDE_PROJECT_DIR`). Write the table to `plugin-hook-triage.md`. Known INTERNAL (do not ship): `adopt-orchestrator-prompts.sh`, `runtime-bridge-dispatch.sh`, `inject-subagent-digest.sh`/`warn-subagent-report.sh` (if orchestration-internal), `worktree-setup.sh`. Known SHIP candidates: `inject-session-bootstrap.sh` (→ becomes the bootstrap injector, project+plugin mix), `inject-matching-rule.sh` (reads **consumer** `.claude/rules/` → project-data), `check-doc-authority.sh` (audits **consumer** files → project-data), `validate-prompt.sh`, `ask-question-reminder.sh`, `end-of-turn-reminder.sh`.
...
- [ ] **Step 4: Copy the two consumer-facing skills** into `plugin/skills/` (reconcile with `extension.json` declared set — `rules-as-tests` is already declared; add `tool-bootstrapping` only if consumer-facing per its SKILL.md).
...
- [ ] **Step 2: Triage agents (T-PLUG-B).** SHIP (consumer-facing per `extension.json` + CLAUDE.md Artifact Ownership Contract): `review-sidecar.md`, `living-docs-auditor.md`, `compliance-verifier.md`. INTERNAL (do not ship): `aif-init.md`, `manual-rule-liveness-prober.md`, `memory-codification-auditor.md`, `orchestrator-worker-discipline.md`, `shipped-agent-liveness-prober.md` — confirm each against the contract; record the call.
- [ ] **Step 3: Copy the SHIP subset** into `plugin/agents/`.
- [ ] **Step 4: Run principle 21** — `npx vitest run packages/core/principles/21-shipped-agent-tools-valid.test.ts`; ensure it covers `plugin/agents/` (extend its glob if needed). Expected: green; no unresolvable tool names (the #551 class).
