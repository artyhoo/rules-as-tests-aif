...
> **⚠ SUPERSEDED (2026-06-24, same day as authoring).** The premise below — that generated declarative rules have NO path into a consumer's running ESLint config (the session brief's "ROOT blocker") — was already stale when written: the `generator-require-composite-tier` umbrella had shipped delivery via `restricted-syntax-audit-exempt` (exempt-aware wrapper + hand-inlined template selectors) AND the dangling-ref guard ([`packages/core/principles/25-template-rule-ref-resolution.test.ts`](../../../packages/core/principles/25-template-rule-ref-resolution.test.ts)). The one *real* residual gap — the hand-inlined selectors were an unguarded 3rd copy that could silently drift from the recipes — was closed by [`packages/core/principles/26-template-selector-sync.test.ts`](../../../packages/core/principles/26-template-selector-sync.test.ts) (PR #716), **not** by the load-time helper-merge seam designed below. **Do not implement the seam below.** The §2 ESLint flat-config no-merge analysis remains correct as reference (it informed principle 26). Root cause of the stale premise: designed against a worktree base, not fresh `origin/staging`.
...
- **Generated plugin modules (`rules-as-tests/<gid>` per rule):** G3b codegen — generating actual rule *modules* — is the umbrella's deliberately-sequenced **destination**, not a forbidden zone. The umbrella lays the rails (S0 engine-assignment matrix: which rules need which engine; S2 engine seam) and builds the codegen itself as its own later stage on that evidence ([`s4-dispatch.md:52`](../../../.claude/orchestrator-prompts/generator-require-composite-tier/s4-dispatch.md) §OUT «no functional G3b codegen»; [`DISPATCH-PROMPT-overnight.md:11`](../../../.claude/orchestrator-prompts/generator-require-composite-tier/DISPATCH-PROMPT-overnight.md) «G3b кодоген здесь НЕ строится — только scope + engine-шов»). Emitting modules **now**, inside a delivery-seam task, front-runs that stage — and it does not even serve today's rules (R13/R18 are declarative `no-restricted-syntax`, not modules). It also changes the `emit()` contract with a broad snapshot blast radius. Approach 1 is the better foundation precisely because it is *on the road to* G3b (§3.1).

### 3.1 This seam is the on-ramp to G3b codegen

The umbrella exists to prepare G3b — generated rule *modules* for rules that exceed `no-restricted-syntax`. Approach 1 is chosen to **serve that trajectory**, not just today's declarative rules:

- **Today (declarative, Class-3 `eslint-restricted`):** generated `no-restricted-syntax` selectors concat into the template arrays (§4.2) → enforced.
- **G3b era (generated modules):** a module rule surfaces in the snippet as a unique-key `rules-as-tests/<gid>: 'error'` entry → it flows through Approach 1's **additive `ruleBlock`** (§4.1/§4.2 step 4), which already registers the plugin. The delivery channel is therefore **already built by this seam**; G3b adds only (a) emitting the module *files* into `eslint-rules-local/` and (b) regenerating the barrel at synth-time so `customRules` resolves them.
- **G3b-era guard extension (flagged, not built here):** the §4.4 guard scans *templates* (the R8 trigger). When G3b emits module-rules into the snippet, the symmetric check — every `rules-as-tests/*` in the generated snippet resolves to a barrel module — belongs in the synthesizer validator ([`gate-conflict.ts`](../../../packages/core/validator/gate-conflict.ts) already owns snippet-completeness), not a framework principle test (those modules are consumer-side, gitignored, synth-time). Recorded here so the next rail is explicit, not silently assumed.
