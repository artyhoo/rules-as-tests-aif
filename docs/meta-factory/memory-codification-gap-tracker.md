...
|---|---|---|---|
| 16 | `check_decided_status_before_recommending` | `.claude/rules/*` (session-discipline: grep memory + decision docs before recommending) | PENDING |
| 17 | `check_inflight_prs_before_building` | `.claude/rules/*` / orchestrator skill (scan `gh pr list --state open` before building a tracked item) | PENDING |
...
| 22 | `orchestrator_verify_state_before_claim` | `.claude/rules/*` (re-verify HEAD/closure before ship / negative-existence claim); §13.34-adjacent | PENDING |
...
| 28 | `verify_diff_direction_before_live_claim` | `.claude/rules/*` (per-file content-probe is mechanical; invocation is judgment) | PENDING |
