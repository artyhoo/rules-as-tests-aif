...
**Sweep verdict for «документация врёт?»:** across ≈158 living docs — **7 real lies found in total** (commit 2: INSTALL flags, opus-override, principles-catalog scope, EP §3.1 husky; commit 3: queue-mode refs, shipped-skill template paths, AI-prompt setup.sh) + 1 maintainer-gated (PROPOSAL Status line, §8 DN). Rules 17/17 mechanism-verified live, agents 8/8, templates/presets/install chain claim-checked clean, goal-docs aligned. Everything else is explicitly dated or deferred-by-marker.
