...
- **Primary cause — design-docs synthesized as reality:** DeepWiki presents PROPOSAL/architecture L0-L5 + «setup.d numbered layers 05-99» as operational subsystems. Reality: `setup.d/` = 3 files (`engine.sh`, `companions.manifest`, `bridge-guided.sh`); `packages/meta-factory/src/*` = skeleton index.ts stubs. The repo docs DO carry FROZEN/authority markers (PROPOSAL.md:9 «**FROZEN** — …», architecture.md header) — the synthesis ignores blockquote authority headers.
