...
| N10? (candidate, R-phase) | **meta-orchestrator prior-art survey** — does any companion (Superpowers / aif.cutcode.dev / aif-handoff / OhMyOpencode / CC-native) implement the `/meta-orchestrator` functional spec (§7 of the kickoff)? Output: ADOPT / ADAPT / BUILD verdict + integration sketch. **Constraint:** project-scope skill, one-button-installable (N6b), substrate-pure, no-paid-LLM-in-CI. The §7 spec is the contract — whatever path is chosen must deliver it. | 🔲 **proposed 2026-05-23, NOT admitted** — kickoff written, **maintainer launches in fresh session** (frustration-trigger: "не чтобы я все время заново просил"). No code in this wave; only the research patch. **Startable now** (no blockers). | brief `.claude/orchestrator-prompts/meta-orchestrator-prior-art/kickoff.md` (§7 = full functional spec) |
| N9? umbrella (candidate) | **guard-liveness** — prove every rule's `bad` corpus trips its real guard, across all 4 `check.type` kinds, change-scoped + periodic full-sweep. **5 sub-waves:** v0 audit (R-phase) · v1 ESLint · v1.5 command/script · v3 manual via Superpowers dogfood · v2 full-sweep CI regression. **3-stage sequencing:** Stage 1 (v0 ‖ v1, start now) → Stage 2 (v1.5 ‖ v3, after v0) → Stage 3 (v2, after v1+v1.5+v3 all merged). Wall-clock ≈10–15 days parallelized, 15–20 sequential. | 🔲 **proposed 2026-05-23, NOT admitted** — full umbrella planned 2026-05-23. v0 + v1 **startable now** (no blockers: engine `gate-rule-tester.ts`, manifest, principle-02, Superpowers v5.1.0 all in place). All 5 kickoffs written. Number/admission = maintainer call | design `research-patches/2026-05-23-guard-liveness-gate.md` §3 umbrella table + §4 dependency graph; kickoffs: `guard-liveness-v0-audit/`, `guard-liveness-gate/` (v1), `guard-liveness-v1.5-cmd-script/`, `guard-liveness-v3-manual-sp/`, `guard-liveness-v2-fullsweep/` |
