...
- **`universalization-fix`** (audit follow-on, M) — layout-coupling residue from #511; **S1/S2/S4 ✅ merged** (#515/#517/#514), **only S3 remains**: single `resolve-source-roots` seam (R-phase) replacing N hardcoded `src/` roots. Kickoff authored (gitignored). Source: project-philosophy audit 2026-06-14.
- **`inject-layer-extension`** (R-phase, S) — DN-3 from doc-audit-ship-boundary (#374); planned kickoff, **deferred** — startable.
- *(The three umbrellas that were the 2026-06-11 frontier all closed 2026-06-13; the 2026-06-14 audit surfaced `universalization-fix` — now S1/S2/S4-merged (#515/#517/#514) with only S3 remaining.)*
