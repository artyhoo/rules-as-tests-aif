...
## §0 — Verified status snapshot (reconciled 2026-06-02)
...
| **`aif task-isolation` / worktree-gap** | why per-task worktrees never materialize (R-phase) | 🟡 **R-phase Findings A+F landed (#360); correction in flight** — **PR #372 OPEN** (corrects #360 Finding A); parallel-dispatch §5 RESOLVED via `use_subagents` (#377/#380) | #360 · #362 (ensure-parallel self-heal) · #377/#380 · **#372 OPEN** |
...
**What actually remains:** apply N2 verdicts (maintainer-owned rule/SSOT edits); **N7 applied + verified** — sole open item = one organic end-to-end SDD run (deferred) · **N8 A-phase C5 + cost-levers** (C1–C4 ✅ SHIPPED; cost-levers gated on §5.3 utilisation trigger) · **`meta-orch-no-arg-overview` Stage 3 UX** (Stages 0/1/2/4 ✅ — Stage 1 §0-refresh is the manual-reconciliation loop, ongoing) · **Track M.5/M.6** (M.1/M.4 ✅; mutation-discipline umbrella ✅) · N4b H10 (deferred, trigger-gated) · **N6b** (NOT blocked — deps met, deprioritized) · N5 (blocked on N7 live-dogfood).

**Open frontier (live, as of 2026-06-02 — derived from `gh` + completion-filter cross-check; the genuinely-open work, distinct from the ~80 historically-closed candidates the completion-filter cannot tag):**
- **`aif-worktree-gap` / `aif-isolation-rphase`** — **PR #372 OPEN** (corrects #360 Finding A). The hottest thread; only candidate with an open PR.
- **`inject-layer-extension`** (R-phase, S) — DN-3 from doc-audit-ship-boundary (#374); planned kickoff, **deferred** — startable.
- **`one-click-installer`** (S) — spec + plan + kickoff READY (2026-05-31), **not yet dispatched**.
- **`f2-aif-fire-backend-iphase`** (I-phase, S) — F2 DN-1=A+D resolved 2026-06-01 (honest `AifFireBackend` dispatchOnly + operator runbook); **I-phase not started**.
