...
### 13.30 Principles 03/05/06/07 mutation coverage (deferred — Wave 8 audit residue)

Research-patch [`2026-05-11-§13.29-substantive-compliance-research.md`](research-patches/2026-05-11-§13.29-substantive-compliance-research.md) gap-enumeration table noted: principles 03, 05, 06, 07 have weaker mutation coverage on positive arms compared to principles 01/02/04/08/09 which have explicit mutation arms. Wave 8 closure (§13.29) addressed substance gaps at the CI/hook layer but did NOT extend mutation-adequacy audit to these four principle tests.

**Status:** armed.

**Trigger to revisit:** any incident where one of principles 03/05/06/07 was satisfied syntactically but failed substantively; OR a separate session takes up «mutation-adequacy-on-principles» as scope (likely absorbed by §13.31 / Wave 9).
...
**Trigger to revisit:** Wave 8 (§13.29) merges complete AND maintainer commits to Phase 9+ session scope.

**First action upon revisit (per kickoff scaffold):** [`.claude/orchestrator-prompts/wave-9-discipline-theatre-audit/kickoff.md`](../../.claude/orchestrator-prompts/wave-9-discipline-theatre-audit/kickoff.md) — research session enumerates surfaces per category, runs sample audit (10-20 surfaces / category), reports adversarial findings + closure mechanism proposals. Implementation deferred to per-category sub-waves AFTER research approved.
...
Wave 9 (§13.31) audits **behavioral compliance** — whether implemented checks substantively enforce their claimed discipline. It does NOT audit **foundational adequacy** — whether the chosen mechanisms reflect best current understanding of the problem domain.

Every load-bearing decision in the project — PROPOSAL.md theses, EXECUTION-PLAN architectural choices, R1-R20 rule set, principles-as-tests pattern, hook substance arms, CI gate design, AI-agnostic sub-agent approach, calibration windows, SSOT format — was made with **internal reasoning + context7 (library docs only)**. External literature, academic research, industry papers, related project comparisons (beyond AIF) were **never** part of the research methodology until Wave 9 §5.0 introduces it for the first time.

This is a project-wide methodology gap from day one, not a Wave-N-localized issue. The discipline-theatre antipattern Wave 9 is auditing **likely originates** in this gap: narrow research → narrow solutions → AI agents game the form → theatre.

**Audit streams (R-phase must enumerate within each):**

1. **A1 — Architectural foundations.** Core thesis «documents lie; tests don't»; recursive self-application invariant; rules-as-tests parade; six-layer architecture. Is each validated, refined, or refuted by external SE literature (executable specifications, literate programming, contract programming, BDD/TDD research)?
2. **A2 — Mechanism choices.** R1-R20 rule set; 10 principles; pre-push §1-§9 sections; CI gate patterns; SSOT format. Each mechanism: chosen as obvious local solution, or evidenced against external alternatives?
3. **A3 — AI-agnostic boundary discipline.** Project claims AI-agnostic surface (`agents/compliance-verifier.md`, `INSTALL-FOR-AI.md` three-layer model, AGENTS.md vocabulary) while internally relying on Claude Code features (`.claude/skills/`, hooks, Skill-tool semantics). **Where is the boundary, is it cleanly maintained, where does Claude-specific creep into AI-agnostic artefacts?** Currently no executable check enforces this — only convention.
4. **A4 — AIF integration depth re-evaluation.** Six SSOT entries (`prior-art-evaluations.md` #8, #27, #28, #29, #30, #31, #32) currently classify lee-to/ai-factory primitives with verdicts ADOPT / ADAPT / DEFER. Each verdict has a `Trigger to revisit`. Phase 10 re-validates: are the verdicts still correct? Has AIF 2.x evolved? Should integration depth increase (e.g., DEFER → ADAPT) given Phase 10 findings on autonomous-orchestration readiness?
5. **A5 — External tooling integration.** For each capability built custom (mutation testing, audit-scripting, hook orchestration), check if a production-grade tool already does it. Stryker is in stack — extend coverage vs build parallel? Are there equivalents for shell-script mutation, lint-rule false-negative measurement, AI-agent compliance frameworks?
6. **A6 — Documentation artefacts as discipline-bearing decisions.** Documentation in this project is **the discipline itself** — there are no executable tests on doc content, only structural checks (principle 09 = header presence). Each doc encodes load-bearing decisions made with internal-reasoning + context7 only:
   - **Root contracts:** [`README.md#why-this-exists`](../../README.md), [`CLAUDE.md`](../../CLAUDE.md), [`INSTALL.md`](../../INSTALL.md), [`INSTALL-FOR-AI.md`](../../INSTALL-FOR-AI.md) (three-layer authority model + override semantics), [`CONTRIBUTING.md`](../../CONTRIBUTING.md), [`AGENTS.md`](../../AGENTS.md) if present.
   - **Rule artefacts:** [`.claude/rules/phase-research-coverage.md`](../../.claude/rules/phase-research-coverage.md) (search-coverage 6-item checklist; §1.7 Forward/Backward pattern; anti-pattern catalogue), [`.claude/rules/doc-authority-hierarchy.md`](../../.claude/rules/doc-authority-hierarchy.md) (Authoritative-for header convention), [`.claude/rules/ai-laziness-traps.md`](../../.claude/rules/ai-laziness-traps.md) (trap catalogue + self-defending mechanism).
   - **Operational restatements:** [`.claude/session-bootstrap.md`](../../.claude/session-bootstrap.md) (Step 0 re-read; goal invariants; reviewer drift-prevention flowchart).
   - **Sub-agent prompts:** [`agents/compliance-verifier.md`](../../agents/compliance-verifier.md) (AI-agnostic §1.7 substance review).
   - **Skills:** [`.claude/skills/*/SKILL.md`](../../.claude/skills/) — auto-trigger patterns, kickoff-author obligations, methodology guidance.
   - **Templates shipped to consumers:** [`packages/core/templates/shared/`](../../packages/core/templates/shared/), preset rules, sub-agent prompts copied via `install.sh`.

   Each doc artefact is a **decision in prose form**. The audit applies: classify (`OWN-BUILD` / `ADAPTED` / `ADOPTED-MECHANISM` / `ADOPTED-VOCABULARY` / `REJECTED`); apply problem-class match check (T16) — are we using ADR-style frontmatter for the problem class ADR is designed for? Is «three-layer authority» a known pattern in SE literature on configuration-as-code, and does our usage match its intent?

   **Documentation is more susceptible to theatre than code** because (a) no mechanical check on prose substance, (b) AI agents read docs first and inherit framings, (c) doc drift cascades silently through downstream edits — pattern observed in 2026-05-09 EXECUTION-PLAN goal-drift incident.

**Scope-reduction discipline: classify before sweeping.** Not every decision needs full academic literature review. Spending Sonnet-time on already-externally-validated patterns is itself a form of theatre («audit-for-the-sake-of-audit»). R-phase **must classify each decision before sweeping**:

| Class | Definition | Audit depth |
|---|---|---|
| **OWN-BUILD** | Designed by this project, no external pattern referenced (e.g. R1-R20 rules, principles 01-10, hook substance arms, six-layer architecture, compliance-verifier prose) | **Full** external evidence sweep (academic + industry + adjacent projects) |
| **ADAPTED** | External pattern modified for project context (e.g. AIF `<!-- handoff:task -->` → `<!-- scope:§N -->`; AIF `/aif` → §13.25 wrapper) | Sweep **on the modifications only**: (a) what we changed + (b) whether changes preserve upstream pattern's validity + (c) **problem-class match** — did modifications drift us to a different problem class than upstream solves? |
| **ADOPTED-MECHANISM** | Used externally-validated mechanism directly (e.g. AIF Step 0 re-read SSOT#8; AIF `/aif` SSOT#31 ADOPT) | **Integration audit** (5 checks): (a) faithful adoption — did we copy the pattern correctly; (b) integration correctness — does it plug into our system; (c) `Trigger to revisit` fired — new evidence to revisit; (d) upstream-drift — has upstream changed (context7, not academic); (e) **problem-class match** — are we using the tool for the problem class it was designed for, not «driving nails with a screwdriver» / `#pattern-matching-on-name`? |
| **ADOPTED-VOCABULARY** | Took only naming/terminology without mechanism (e.g. AGENTS.md spec SSOT#7; AGENTS.override.md SSOT#15; both `ADOPT VOCABULARY`) | **No audit beyond `#pattern-matching-on-name` sanity check** (does our usage match upstream semantics?) |
| **REJECTED** | Considered and chose not to use (e.g. AIF implement-coordinator DEFER SSOT#30; Codex MCP CLI REJECT SSOT#35) | **Rejection rationale revalidation** + `Trigger to revisit` fired-check + upstream-change since rejection |

**Expected distribution across ~120 decision points** (R-phase will refine — A6 expansion adds ~25 doc-artefacts):
- OWN-BUILD: ~65 (most rules/principles/mechanisms + most doc-encoded decisions invented in this project: §1.7 pattern, Forward/Backward checklist, Authoritative-for header convention, three-layer authority model, AI-laziness-traps catalogue, etc.)
- ADAPTED: ~10 (several Wave 7-8 hybrids; possibly Authoritative-for if inspired by ADR/MADR)
- ADOPTED-MECHANISM: ~7 (AIF Step 0, `/aif`, etc., + AGENTS.md if used as canonical mechanism vs vocabulary)
- ADOPTED-VOCABULARY: ~8 (AGENTS.md spec, AGENTS.override.md, FROZEN-doc pattern naming)
- REJECTED: ~20 (SSOT DEFER/REJECT rows)

Classification discipline applies **uniformly to A1-A6** — code-encoded decisions AND doc-encoded decisions get the same audit-depth-per-class treatment.

Full academic sweep applies only to ~75 decisions (OWN-BUILD + ADAPTED + selective ADOPTED-MECHANISM). **~35-40% scope reduction** vs. uniform full-sweep approach.

**AIF-chain risk (must mitigate, not skip):** for ADOPTED items, audit must include the question «did the upstream source (e.g. AIF) itself externally validate this pattern, or did THEY context7-only it?» If upstream itself is unvalidated, the adoption inherits the gap and the item promotes to OWN-BUILD-class audit depth. **Default assumption: lee-to/ai-factory's own design decisions have not been externally validated either** (single-maintainer community project; high probability of same context7-only methodology); Phase 10 confirms or refutes this per ADOPTED item.

**Methodology output: decision register.** Per classified decision: retroactive external-evidence sweep (depth per class above), verdict (`CONFIRMED` / `REVISE` / `DEPRECATE` / `GAP-NOTED`). For REVISE/DEPRECATE: targeted sub-waves. For CONFIRMED: mark SSOT entry `externally validated YYYY-MM-DD`. For GAP-NOTED: armed trigger.

**Scope explicitly admits full-refactor outcomes.** Phase 10 is NOT scoped to «cosmetic fixes». If audit surfaces that core thesis needs refinement, or that a key mechanism duplicates existing production-grade tool, REVISE/DEPRECATE verdicts apply. Maintainer must be ready for the possibility that 4-6 weeks of audit triggers further weeks of foundational rework.

**Permanent rule (deliverable):** new requirement in `.claude/rules/phase-research-coverage.md` — «external evidence sweep mandatory for any discipline-bearing decision» (when problem domain has academic/industry name); companion principle test enforcing it on R-phase docs going forward.

**Recursive self-application check (must be in scope):** Phase 10 itself is a discipline-bearing artefact. Its methodology (decision register + external evidence) must apply to itself. Phase 10 output §N includes: «did Phase 10 audit Phase 10? what would auditing Phase 10 look like? did the sub-team running Phase 10 themselves apply external evidence or pattern-match this kickoff?»

**Status:** armed.

**Trigger to revisit:** Wave 9 (§13.31) closes (R-phase output approved + per-category sub-waves merged) AND maintainer commits to Phase 10 scope including possibility of foundational refactor.

**Scope warnings:**
- Phase 10 may surface that core thesis needs refinement → potential PROPOSAL.md authoritative-revision + README.md goal-statement revision (both currently maintainer-owned-only per Artifact Ownership Contract).
- Phase 10 may surface that 2-3 key mechanisms duplicate existing production-grade tools → REVISE = significant rewrite of test/audit infrastructure.
- Phase 10 may itself become theatre if Sonnet does perfunctory web searches and uniformly stamps CONFIRMED → paired-negative methodology test required (seeded-gap probes: deliberately seeded suboptimal decisions to verify methodology catches them).
- **Moratorium consideration:** during Phase 10 R-phase, freeze new major discipline-bearing decisions to avoid auditing-moving-target.
- 4-6 weeks of meta-work with no shipped framework features; maintainer decides whether sequencing fits roadmap.

**Risks of NOT doing Phase 10:** project continues building on foundation that has never been validated against external evidence; discipline-theatre at architectural layer remains uncaught; recursive-self-application invariant remains aspirational (we apply it to user code but not to our own foundational design choices).

**First action upon revisit:** draft `.claude/orchestrator-prompts/phase-10-foundations-audit/kickoff.md` modeled on Wave 9 kickoff (§5.0 literature sweep mandatory; §6 AI laziness traps; recursive self-application clause). R-phase output: `docs/meta-factory/research-patches/2026-MM-DD-§13.32-foundations-audit-research.md` with decision register + verdict matrix + sub-wave proposals.

**Owner:** maintainer + Phase 10+ planning session (post-Wave-9).

**Cross-references:** §13.29 (Wave 8 closure — CI/hook substance layer); §13.31 (Wave 9 umbrella — behavioral compliance layer; Phase 10 fires after Wave 9); [`.claude/orchestrator-prompts/wave-9-discipline-theatre-audit/kickoff.md §5.0`](../../.claude/orchestrator-prompts/wave-9-discipline-theatre-audit/kickoff.md) (literature sweep introduced for first time); SSOT entries #8, #27, #28, #29, #30, #31, #32 in [`prior-art-evaluations.md`](prior-art-evaluations.md) (AIF integration verdicts subject to A4 re-evaluation); `agents/compliance-verifier.md` + `INSTALL-FOR-AI.md` (AI-agnostic surface, subject to A3 audit).

**Origin:** maintainer 2026-05-12 surface during Wave 8.4 PR creation — «возможно слишком узкий [ресёрч] и возможно приняты не лучшие решения … все правила и все их релизации прям ВЕСЬ проект». Followed by recognition that Wave 9 C6b («research methodology adequacy») partially captures this but is scoped to R-phase docs only; full foundations audit requires separate phase.
