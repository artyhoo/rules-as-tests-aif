...
# ─── Helpers ─────────────────────────────────────────────
# All helpers (copy_safe, mkdir_safe, chmod_safe, detect_pm, transform_internal_refs,
# merge_prettierignore, copy_skill_with_transform, refresh_skill_with_transform,
# patch_stryker_package_manager, ignore_shipped_configs, etc.) live in setup.d/lib.sh.
# UPSTREAM_BLOB_URL is also declared there.
...
# transform_internal_refs() (defined in lib.sh) without running the install pipeline
...
# ─── Layer execution ─────────────────────────────────────
# O2: layers sourced into this shell (not subshelled) so SKIPPED/globals persist.
# Fork #2 (dispatcher style) PARKED: provisional glob loop (lexicographic order).
# Alternative: explicit ordered call-list. Operator decides.
