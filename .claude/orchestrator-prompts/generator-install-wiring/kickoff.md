...
2. **Generalize the wirer.** `wire-eslint-r2.ts` (single rule) → an N-rule wirer that ingests the synthesizer's emitted `rules` object and AST-merges it into the consumer's resolved ESLint config, idempotently and non-destructively. R2 becomes a special case. **Watch flat-config override semantics:** the preset base block already configures `restricted-syntax-audit-exempt` (e.g. a `TSEnumDeclaration` selector) — merge into the existing entry's selector array, do NOT append a sibling config object that clobbers it (last-wins per file).
