...
<!-- spec-of: agents/best-practices-sidecar.md -->
<!-- Most of best-practices-sidecar's checks (R1-R3, R5-R8, R12, R14-R16, R19-R20) are
     already enforced earlier: custom ESLint rules at edit-time + tsc/depcruise at pre-push.
     Only the checks below have NO earlier deterministic channel, so they ship here as a
     skill-context augmentation rather than as a colliding agent (C-1 KEEP-AIF resolution).
     Portable reference: agents/best-practices-sidecar.md (kept in-repo, not installed). -->
